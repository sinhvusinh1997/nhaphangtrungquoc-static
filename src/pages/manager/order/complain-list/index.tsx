...
import {ComplainListFilter, ComplainListForm, ComplainListTable, Layout, showToast} from "~/components";
...
const Index: TNextPageWithLayout = () => {
	...
	const handleExportExcel = async () => {
		try {
			const res = await complain.exportExcel();
	...
	return (
		<div className="tableBox">
			<div className="pb-4">
				<ComplainListFilter handleFilter={handleFilter} handleExportExcel={handleExportExcel} />
			</div>
			<ComplainListTable
				filter={filter}
			...
			/>
			<ComplainListForm onCancel={() => setModal(false)} visible={modal} defaultValues={item.current} />
		</div>
	);
};
