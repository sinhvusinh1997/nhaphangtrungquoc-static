...
import {ClientListFilter, ClientListForm, ClientListTable, IconButton, Layout, showToast, toast} from "~/components";
...
const Index: TNextPageWithLayout = () => {
	...
	const {data: dathangList} = useQuery(["dathang"], () => user.getList({UserGroupId: 4, IsEmployee: 1}), {
		onSuccess: (res) => res?.Data?.Items,
		enabled: !!newUser,
	});

	const {data: saleList} = useQuery(["sale"], () => user.getList({UserGroupId: 7, IsEmployee: 1}), {
		onSuccess: (res) => res?.Data?.Items,
		enabled: !!newUser,
	});
	...
	return (
		<div className="tableBox py-2">
			<div className="xl:grid grid-cols-4 gap-4 mb-4">
				<div className="col-span-3 xl:mb-0 mb-2">
					<ClientListFilter handleFilter={handleFilter} dathangList={dathangList} saleList={saleList} />
				</div>
