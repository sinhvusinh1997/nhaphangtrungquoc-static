...
import {BonusManagementFilter, BonusManagementTable, Button, FormCard, Layout, toast} from "~/components";
...
const Index: TNextPageWithLayout = () => {
	...
	const mutationPayment = useMutation((data: any) => staffIncome.payment({Type: 2}, data), {
		onSuccess: () => {
	...
	const _handlePayAll = async (data: any) => {
		await mutationPayment.mutateAsync(data);
	};

	const mutationPaymentOne = useMutation((Id: number) => staffIncome.payment({Type: 1, Id}, data), {
		onSuccess: () => {
			toast.success("Thanh toán thành công");
			refetch();
		},
	...
	return (
		<>
			...
			<Modal visible={isModalOpen} closable={false} footer={false}>
				<FormCard>
					<FormCard.Header onCancel={() => setIsModalOpen(false)}>Thông báo!</FormCard.Header>
					<FormCard.Body>
					...
					<FormCard.Footer>
						<Button title="Hủy" onClick={() => setIsModalOpen(false)} btnClass="!bg-pending" />
						<Button
							title="Thanh toán"
							btnClass="!bg-active"
							disabled={data?.Items[0]?.MaxTotalPriceReceiveNotPayment ? false : true}
							onClick={() => _handlePayAll({PageIndex: 0, PageSize: 0})}
						/>
					</FormCard.Footer>
				</FormCard>
			</Modal>
...
Index.breadcrumb = breadcrumb.employee.bonusManagement;
Index.Layout = Layout;
