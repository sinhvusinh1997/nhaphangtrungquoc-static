...
const Index: TNextPageWithLayout = () => {
	...
	const {
		...
		isFetching,
		isError,
	} = useQuery(["clientRechargeData", {...filter}], () => adminSendUserWallet.getList(filter).then((res) => res.Data), {
		onSuccess: (data) => {
	...
	const handleExportExcel = () => {
		try {
			adminSendUserWallet.exportExcel().then((res) => {
		...
		<>
			<div className="grid grid-cols-4 gap-2 mb-4">
				<div className={boxTop}>
					Tổng đơn
					<span className="text-bold text-blue font-semibold text-[20px]">{userRechargeData?.TotalItem}</span>
				</div>
				<div className={boxTop}>
					Số đơn đã duyệt
			...
			<div className="tableBox">
				<div className="pb-4">
					<RechargeHistoryFilter handleFilter={handleFilter} handleExportExcel={handleExportExcel} />
				</div>
				<RechargeHistoryTable
					{...{
				...
				/>
				<RechargeHistoryForm visible={modal} onCancel={() => setModal(false)} defaultValues={item.current} />
			</div>
		</>
	);
};
