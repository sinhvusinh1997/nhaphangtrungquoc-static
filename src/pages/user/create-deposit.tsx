...
import {CreateDepositSelect, CreateDepositTable, IconButton, toast, UserLayout} from "~/components";
...
const Index: TNextPageWithLayout = () => {
	...
	const {control, reset, handleSubmit, setValue} = useForm<TUserCreateDeposit>({
		mode: "onBlur",
	});
	...
	const mutationAdd = useMutation((data: TUserCreateDeposit) => transportationOrder.create(data), {
		onSuccess: () => {
	...
	const _onPress = (data: TUserCreateDeposit) => {
		...
		if (flat) {
			delete data.UID;
			mutationAdd.mutateAsync({...data, UID: user.UserId});
		}
	};
