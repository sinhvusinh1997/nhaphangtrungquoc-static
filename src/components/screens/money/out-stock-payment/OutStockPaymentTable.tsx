...
const OutStockPaymentTable: React.FC<TTable<TOutStockSession> & TProps> = ({data, loading, filter, handleFilter}) => {
	const columns: TColumnsType<TOutStockSession> = [
		...
		},
		{
			dataIndex: "Status",
			title: "Trạng thái",
			render: (status, record) => <Tag color={status === 1 ? "#cf1322" : "#389e0d"}>{record.StatusName}</Tag>,
			responsive: ["xl"],
		},
