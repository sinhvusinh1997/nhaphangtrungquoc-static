...
import {FormSelect, FormTextarea, IconButton, showToast, toast} from "~/components";
import {EFormalPaymentData, EPaymentTypeData, formalPaymentData, paymentTypeData} from "~/configs/appConfigs";
...
export const OrderPaymentForm: React.FC<TProps> = ({data, loading}) => {
	const {query} = useRouter();

	const {handleSubmit, control, reset, getValues, setValue} = useForm<TPaymentOrder>({
		mode: "onBlur",
	});
	...
	const _onPress = async (data_pay: TPaymentOrder) => {
		const amountValue = getValues("PaymentType") === 1 ? data?.AmountDeposit : data?.RemainingAmount;
		...
		<React.Fragment>
			<div className="grid grid-cols-2 gap-4 mb-6">
				<div className="col-span-2">
					<p className="text-[16px] font-semibold text-[#585858]">Thông tin phí</p>
				</div>
				<div className="infoPayment bg-[#f8f8f8] p-4">
					<div className="mb-4">
						<div className="flex justify-between pb-2">
							<div className="tracking-wide">
								<Skeleton paragraph={{rows: 1, width: 180}} title={false} loading={loading} active>
									Mã đơn hàng
								</Skeleton>
							</div>
							<div className=" text-sm font-semibold text-[#585858]">
								<Skeleton paragraph={{rows: 1, width: 80}} title={false} loading={loading} active>
									{"# "}
									{data?.Id}
								</Skeleton>
							</div>
				...
				<div className="payment px-2 ">
					...
					<div className="grid grid-cols-2 gap-2 mt-4 items-center justify-between">
						...
						<div className="col-span-1">
							<Skeleton paragraph={{rows: 1, width: "100%"}} title={false} loading={loading} active>
								{data?.Deposit > 0 ? (
									<FormSelect
										placeholder=""
										control={control}
			...
			<div className="flex">
				<Skeleton paragraph={{rows: 1, width: 180}} title={false} loading={loading} active>
									...
									content: (
										<div className="mt-6">
											<div className="flex justify-between mb-3">
												<span>Loại thanh toán: </span>
												<span className="font-bold">{getValues("PaymentType") === 1 ? "Đặt cọc" : "Thanh toán"}</span>
											</div>
											<div className="flex justify-between mb-3">
												<span>Phương thức thanh toán: </span>
												<span className="font-bold">
													{getValues("PaymentMethod") === 1 ? "Trực tiếp" : "Ví điện tử"}
												</span>
											</div>
											<div className="flex justify-between mb-3">
											...
											<b className="mt-10 block">Xác nhận thanh toán kiện hàng này?</b>
										</div>
									),
					...
					<IconButton onClick={() => router.back()} icon="fas fa-reply-all" title="Trở về" showLoading toolip="" />
				</Skeleton>
			</div>
		</React.Fragment>
	);
