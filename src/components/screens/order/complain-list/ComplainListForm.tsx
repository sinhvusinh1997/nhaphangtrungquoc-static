...
export const ComplainListForm: React.FC<TForm<TReport>> = ({onCancel, visible, defaultValues}) => {
	...
	useDeepEffect(() => {
		if (visible) {
			reset({
				...defaultValues,
				AmountCNY:
					defaultValues?.Amount && defaultValues?.CurrentCNYVN && defaultValues?.Amount / defaultValues?.CurrentCNYVN,
			});
		}
	}, [visible]);
	...
	const mutationUpdate = useMutation((data: TReport) => complain.updateComplain(data), {
	...
	return (
		<Modal visible={visible} width={1000} onCancel={onCancel}>
			<FormCard loading={isFetching}>
				...
				<FormCard.Body>
					<div className="lg:grid lg:grid-cols-4 gap-3">
						...
						<div className="col-span-4 mb-2 lg:mb-0">
							<FormUpload control={control} name="IMG" required={false} disabled />
						</div>
					</div>
				</FormCard.Body>
				<FormCard.Footer>
