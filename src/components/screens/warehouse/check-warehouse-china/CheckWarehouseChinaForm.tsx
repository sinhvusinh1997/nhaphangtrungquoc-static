...
import {CheckWarehouseChinaNewBagForm, CheckWarehouseChinaNewCodeForm, CheckWarehouseChinaTable} from ".";
...
export const CheckWarehouseChinaForm = () => {
	...
	const {control, handleSubmit, getValues, reset, resetField, setValue} = useForm<TWarehouseCN>({
		mode: "onBlur",
		defaultValues: {
		...
		if (!Object.keys(watchArray()).length) {
			...
			if (watchArray().hasOwnProperty(key)) {
				...
				if (!diffData.length) {
					...
					setValueArray(key, [...newData.filter((item) => !!diffData.find((x) => x === item.Id)), ...watchArray(key)]);
				}
			} else {
				setValueArray(key, newData);
			}
	...
	const _onCreate = (newData: TWarehouseCN) => {
		...
		queryClient.fetchQuery(["barCodeCN", newData.OrderTransactionCode], () =>
			smallPackage
				.getByTransactionCode({TransactionCode: newData.OrderTransactionCode.trim()})
				.then((res) => {
	...
	return (
		<Spin spinning={loading || mutationUpdate.isLoading || mutationAddOrderTransactionCode.isLoading}>
			...
			<div className="flex items-end mt-4 mx-4">
				<FormInput
					...
					inputClassName="barcode mb-4 xl:mb-0"
					prefix={
			...
			<div className="xl:flex mt-4 mx-4">
								...
								[
									{
										...
										cb: handleSubmitArray((data) =>
											_onPress(Object.values(data).reduce((prev, cur) => [...prev, ...cur], []))
										),
									},
								],
								{}
				...
				>
					<IconButton icon="far fa-plus" title="Thêm mã kiện (Ctrl + M)" onClick={() => setModalCode(true)} toolip="" />
				</HookWrapper>
			</div>
			<CheckWarehouseChinaNewBagForm visible={modalPackage} refetch={refetch} onCancel={() => setModalPackage(false)} />
			<CheckWarehouseChinaNewCodeForm
				visible={modalCode}
				onCancel={() => setModalCode(false)}
				newOrderTransactionCode={newOrderTransactionCode.current}
				handleData={(newData: TWarehouseCN[], key: string) => handleData(newData, key)}
			/>
