...
export const OutStockFormDetail: React.FC = () => {
	...
	const {data, isError, isFetching, refetch} = useQuery(["outStockDetail"], () => outStockSession.getByID(+query?.id), {
		onSuccess: (data) => data,
		onError: toast.error,
		retry: false,
	...
	const mutationExport = useMutation(() => outStockSession.export({Id: data?.Data?.Id}), {
		onSuccess: (res) => {
	...
	if (isError) return <Empty description={`Không tìm thấy hoá đơn xuất kho #${query?.id}`} />;
