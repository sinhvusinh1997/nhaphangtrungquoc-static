...
export const OutStockTable: React.FC<TTable<TWarehouseVN> & {UID: number}> = ({data, UID}) => {
					...
					<a
						...
						onClick={() =>
							router.push({
								...
								query: {id: record?.MainOrderId ? record?.MainOrderId : record?.TransportationOrderId},
							})
						}
					>
		...
		expandedRowRender: (record, index) => (
			<ul className="px-2 text-xs">
				...
				<li className="xl:hidden justify-between flex py-2">
					...
					<div>
						<Tag color={smallPackageStatusData.find((x) => x.id === record.Status)?.color}>
		...
		<div className="mt-4">
			...
			<DataTable
				{...{
					...
					rowSelection: {type: "checkbox", onChange: (val) => setIdsExport(val)},
				}}
			/>
		</div>
	);
