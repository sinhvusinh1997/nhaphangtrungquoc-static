...
export const OutStockFormTableDetail: React.FC<TTable<TOutStockSessionPackages> & {totalMustPay; dataAll}> = ({
	data,
	loading,
	totalMustPay,
	dataAll,
	...
	const columns: TColumnsType<TOutStockSessionPackages> = [
		...
		},
		{
			...
			title: "Cân thực (kg)",
			align: "right",
			render: (smallpackage: TSmallPackage) => smallpackage.Weight && _format.getVND(smallpackage.Weight, ""),
		},
		{
			dataIndex: "SmallPackage",
			align: "right",
			...
			render: (_, record, index) => {
				return `${record.SmallPackage.Length ?? 0} x ${record.SmallPackage.Width ?? 0} x ${
					record.SmallPackage.Height ?? 0
				}`;
			},
			responsive: ["lg"],
		},
		...
		},
		{
			...
			render: (record) => {
				return <Tag color={record ? "#388E3C" : "#D32F2F"}>{record ? "Đã thanh toán" : "Chưa thanh toán"}</Tag>;
			},
		},
		{
			...
			<>
				<Table.Summary.Row>
					...
					<Table.Summary.Cell index={1} colSpan={1} align="right">
						<Text type="danger">
							{_format.getVND(
								data.reduce((prev, cur) => prev + cur?.SmallPackage?.PayableWeight, 0),
								" KG"
							) || "0 KG"}
						</Text>
					</Table.Summary.Cell>
