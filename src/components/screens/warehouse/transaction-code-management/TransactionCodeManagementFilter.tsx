...
export const TransactionCodeManagementFilter: React.FC<TProps> = ({handleFilter, handleExporTExcel}) => {
	...
	return (
		<div className="xl:grid xl:grid-cols-5 xl:gap-4 w-full pb-4">
			<div className="col-span-1 xl:mb-0 mb-4">
				<FilterSelect
					...
					label="Tìm kiếm theo"
					isClearable
					handleSearch={(val: ESearchSmallPackageStatusData) => (SearchType.current = val)}
				/>
			</div>
			<div className="col-span-1 xl:mb-0 mb-4">
				<FilterInput {...inputProps} handleSearch={(val: string) => (SearchContent.current = val.trim())} />
			</div>
			<div className="col-span-1 xl:mb-0 mb-4">
				<FilterSelect
					...
					label="Trạng thái"
					isClearable
					handleSearch={(val: ESmallPackageStatusData) => (Status.current = val)}
