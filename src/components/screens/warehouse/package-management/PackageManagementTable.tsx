...
export const PackageManagementTable: React.FC<TTable<TPackage> & TProps> = ({data, loading, filter, handleFilter}) => {
	//Export data excel
	...
	const columns: TColumnsType<TPackage> = [
		...
		},
		{
			...
			render: (status, record) => (
				<Tag color={bigPackageStatusData.find((x) => x.id === status)?.color}>{record.StatusName}</Tag>
			),
			responsive: ["lg"],
		},
		{
		...
		expandedRowRender: (record) => (
			<ul className="px-2 text-xs">
				...
				<li className="lg:hidden justify-between flex py-2">
					...
					<div>
						<Tag color={bigPackageStatusData.find((x) => x.id === record?.Status)?.color}>{record.StatusName}</Tag>
					</div>
				</li>
				...
				<li className="xl:hidden justify-between flex py-2">
					...
					<Space>
						<ActionButton
							...
							title="Cho vào bao lớn"
						/>
						<ActionButton onClick={() => _onExportExcel(record.Id)} icon="fad fa-download" title="Xuất Thống Kê" />
					</Space>
				</li>
			</ul>
		),
