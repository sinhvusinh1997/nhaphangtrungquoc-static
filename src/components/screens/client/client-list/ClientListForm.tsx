...
import {activeData, EActiveData, EGenderData, genderData} from "~/configs/appConfigs";
...
export const ClientListForm: FC<TProps> = ({
	...
	const {handleSubmit, setValue, watch, reset, control} = useForm<TClient & {UserGroupId: number}>({
		mode: "onBlur",
		defaultValues: {
			Gender: 0,
		},
	...
	return (
		<Modal visible={visible} width={900} style={{top: 50}} onCancel={onCancel}>
			<FormCard>
				...
				<FormCard.Body>
					<div className="grid grid-cols-2 gap-4">
						...
						<div className="col-span-1">
							<FormRadio control={control} name="Gender" data={genderData} label="Giới tính" />
						</div>
						<div className="col-span-1">
							<FormInput
								control={control}
						...
						<div className="col-span-1">
							<FormAsyncSelect
								control={control}
								name="UserGroupId"
								data={{options: userGroupCatalogue}}
						...
						<div className="col-span-2">
							<FormSelect
								...
								required={false}
								callback={async () => setValue("IsLocked", watch("Status") === EActiveData.Blocked ? true : false)}
							/>
						</div>
					</div>
				</FormCard.Body>
				<FormCard.Footer>
					<Button title="Thêm mới" btnClass="!bg-active" onClick={handleSubmit(_onPress)} />
