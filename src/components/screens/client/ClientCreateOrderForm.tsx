...
import {FormInput, FormInputNumber, FormSelect, FormUpload} from "~/components/globals/formBase";
...
export const ClientCreateOrderForm = ({userData}: TProps) => {
	...
	const columns: TColumnsType<TUserCreateOrderProduct> = [
		{
			...
			title: "Ảnh đại diện",
			render: (_, __, index) => (
				<FormUpload image={true} control={control} name={`Products.${index}.ImageProduct` as const} />
			),
		},
		{
	...
	const mutationAdd = useMutation(mainOrder.addAnother, {
		onSuccess: () => {
			toast.success(`Bạn đã tạo đơn hàng khác thành công cho user ${userData.UserName}`);
