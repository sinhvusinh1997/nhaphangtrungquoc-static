...
import {FormAsyncSelect, FormDate, FormInput, FormInputNumber, FormRadio, FormSelect} from "~/components";
	...
	const _onPress = (data: TForm) => {
		...
		if (data?.PasswordNew) {
			if (data?.PasswordAgain !== data?.PasswordNew) {
				...
				mutationUpdate.mutateAsync({...newData, UserGroupName: UserGroupNameCur.current});
			}
		} else {
