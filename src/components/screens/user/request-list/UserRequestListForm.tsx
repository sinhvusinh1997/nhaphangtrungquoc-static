...
import {DataTable, FormInput, FormInputNumber, FormTextarea} from "~/components";
...
export const UserRequestListForm: React.FC<TProps> = ({data}) => {
	...
	const columns: TColumnsType<TCreateRequestPaymentOrder & {Desc2: string; Desc1: number}> = [
		{
			dataIndex: "Id",
			title: "Id",
		},
		...
		},
		{
			...
			title: "Giá tiền (VNĐ)",
			align: "right",
			render: (value, record, index) => _format.getVND(record?.Desc1 * data?.Currency, " "),
		},
		{
			dataIndex: "action",
			title: "Ghi Chú",
	...
	return (
		<div
			style={{
				...
				pointerEvents: updateLoading === true || deleteLoading === true ? "none" : "all",
			}}
			className="flex w-full justify-between"
		>
			<div className="tableBox w-[25%] py-2">
				<div className="col-span-1 text-base font-bold py-2 uppercase border-b border-main">Thông tin</div>
			...
			<div className="tableBox w-[73%] h-fit py-2">
				<div className="text-base font-bold py-2 uppercase border-b border-main mb-4">Danh sách chi tiết</div>
				...
				<div className="flex col-span-1 items-start my-2 mt-4">
					<Link href="/user/request-list">
						<a>
							<IconButton onClick={undefined} title="Trở về" icon="fas fa-undo-alt" toolip="" btnClass="!mx-2" />
						</a>
					</Link>
					{
