...
export const HistoryRechargeVNDTable: React.FC<TTable<TUserHistoryRechargeVND>> = ({
	data,
	pagination,
	handlePagination,
	loading,
	...
	const columns: TColumnsType<TUserHistoryRechargeVND> = [
		...
		},
		{
			title: "Trạng thái",
			dataIndex: "Status",
			render: (status) => <Tag color={rechargeStatusData[status - 1].color}>{rechargeStatusData[status - 1].name}</Tag>,
			responsive: ["lg"],
			width: 200,
		},
		{
		...
		},
		{
			...
			render: (_, record) =>
				record?.Status === ERechargeStatusData.Pending && (
					<ActionButton onClick={() => handleModal(record)} icon="far fa-trash-alt" title="Xóa" />
				),
			responsive: ["xl"],
			width: 100,
		},
		...
		expandedRowRender: (record) => (
			<ul className="px-2 text-xs">
				...
				<li className="md:hidden justify-between flex py-2">
					...
					<div>{record.TradeContent}</div>
				</li>
				<li className="md:hidden justify-between flex py-2">
					<span className="font-medium mr-4">Trạng thái:</span>
					<div>
						<Tag color={rechargeStatusData[record.Status - 1].color}>{rechargeStatusData[record.Status - 1].name}</Tag>
					</div>
				</li>
				<li className="md:hidden justify-between flex py-2">
