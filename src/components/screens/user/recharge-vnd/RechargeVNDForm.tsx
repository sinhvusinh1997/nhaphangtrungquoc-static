...
import {BankCard, FormInputNumber, FormSelect, FormTextarea} from "~/components";
...
export const RechargeVNDForm: React.FC<TProps> = ({bankCatalogue}) => {
	...
	const {control, handleSubmit, reset} = useForm<TUserHistoryRechargeVND>({
		mode: "onBlur",
	});

	useDeepEffect(() => reset({BankId: bankCatalogue?.[0]?.Id}), [bankCatalogue]);
						...
						<div className="flex mb-4 items-center">
							<div className="w-full">
								<FormSelect
									...
									name="BankId"
									data={bankCatalogue}
									defaultValue={bankCatalogue?.[0]}
									select={{label: "BankInfo", value: "Id"}}
									placeholder=""
									rules={{required: "This field is required"}}
									label={"Đã chuyển vào tài khoản"}
								/>
							</div>
						</div>
						<div className="flex mb-4 items-center">
