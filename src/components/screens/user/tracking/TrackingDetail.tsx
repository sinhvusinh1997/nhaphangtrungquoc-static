...
export const TrackingDetail: React.FC<TProps> = ({data}) => {
	const handleEmployee = () => {
		...
		} else if (firstItem.Status === ESmallPackageStatusData.ArrivedToChinaWarehouse) {
			employee = firstItem.StaffTQWarehouse;
		} else if (firstItem.Status === ESmallPackageStatusData.ArrivedToVietNamWarehouse) {
			...
			<div className="xl:p-6">
				<div className="rounded-t-sm py-6 border-b border-b-[#e0e0e0]">
					<p className="uppercase">
						...
						<span className="uppercase text-xl font-bold text-[#4E7D48]">{data[0]?.OrderTransactionCode}</span>
					</p>
				</div>
				<div className="flex flex-wrap items-center justify-start">
					<p className="text-sm mr-4 py-4">
						{data[0]?.OrderTypeName}: <span className="font-bold tracking-tighter">{data[0]?.Id}</span>
					</p>
					<p className="text-sm mr-4 py-4">
						Thời gian: <span className="font-bold tracking-tighter">{handleTimer()}</span>
					</p>
					<p className="text-sm mr-4 py-4">
						Trạng thái: <span className="font-bold tracking-tighter">{data[0]?.StatusName}</span>
					</p>
					<p className="text-sm mr-4 py-4">
						Nhân viên xử lý: <span className="font-bold tracking-tighter">{handleEmployee()}</span>
					</p>
				</div>
			</div>
