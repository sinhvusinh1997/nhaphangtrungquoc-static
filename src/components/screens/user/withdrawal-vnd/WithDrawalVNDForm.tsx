...
export const WithDrawalVNDForm: FC = () => {
	...
	const {data, isError, refetch} = useQuery(["articleList", newUser?.UserId], () => user.getByID(newUser?.UserId), {
		onSuccess: (data) => data,
		...
		>
			<div className="">
				...
				<div className="flex items-end m-2 flex-col ">
					...
					<p className="text-sm font-bold text-blue">{_format.getVND(data?.Data?.Wallet)}</p>
				</div>
				<div className="p-4">
					...
					<div className="flex items-center mt-4">
						<FormTextarea control={control} name="Note" label="Nội dung" placeholder="" />
					</div>
				</div>
			</div>
