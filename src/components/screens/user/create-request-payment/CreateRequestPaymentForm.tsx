...
import {FormInput, FormInputNumber, FormTextarea, showToast} from "~/components";
import {IconButton} from "~/components/globals/button/IconButton";
import {useDeepEffect} from "~/hooks";
import {selectFirstPageDashboard, selectUser, useAppSelector} from "~/store";
...
export const CreateRequestPaymentForm = () => {
	...
	const {control, handleSubmit, watch, setValue} = useForm<TCreateRequestPaymentOrder>({
		mode: "onBlur",
		defaultValues: {
	...
	useDeepEffect(() => {
		const sumTotalPrice = watch("PayHelpDetails").reduce((prev, cur) => {
			...
			if ((!isNumber(prev) && !isNumber(cur.desc1)) || (isNumber(prev) && !isNumber(cur.desc1))) {
				return prev;
			}
	...
	useEffect(() => {
		(async () => {
			...
			if (totalPrice) {
				try {
					const currency = await (await payHelp.getExchangeRate(totalPrice)).Data;
	...
	return (
		<div className="xl:grid grid-cols-2 gap-6" style={{pointerEvents: loading ? "none" : "all"}}>
			<div className={clsx(boxContent, "xl:mb-0 mb-4 tableBox max-h-[800px] overflow-hidden")}>
				...
				<div className="col-span-2">
					<div className="max-h-[600px] overflow-y-auto pr-4">
						{fields.map((item, index) => (
							<div key={item?.id} className={clsx("flex", index !== 0 && "mt-4")}>
								<FormInputNumber
									control={control}
								...
								/>
								<div className="w-4" />
								<FormInput control={control} name={`PayHelpDetails.${index}.desc2` as const} placeholder="Nội dung" />
								<IconButton
