...
import {DepositDetail, FormInput, FormInputNumber, FormSelect, FormTextarea, IconButton, toast} from "~/components";
...
export const DepositListForm: React.FC<TProps> = ({
	...
	const {handleSubmit, reset, watch, setValue, control, getValues} = useForm<TUserDeposit>({
		mode: "onBlur",
		defaultValues,
	});
	...
	const handleSetValue = useCallback((key: keyof TUserDeposit, value: any) => setValue(key, value), []);
	...
	const handleSetNewTotal = () => {
		...
		handleSetValue("TotalPriceCNY", Math.round((newTotal / defaultValues.Currency) * 100) / 100);
	};

	return (
		<div className="xl:grid xl:grid-cols-10 gap-4 h-full w-full">
			<div className="tableBox xl:col-span-3 gap-4 h-fit p-5" style={{position: "sticky", top: "80px"}}>
				<div className="col-span-2 text-base mb-4 font-bold py-2 uppercase border-b border-main">Thông tin</div>
					...
					<FormSelect
						...
						defaultValue={
							...
							defaultValues.StatusName && {
								...
								name: orderStatusData.find((x) => x.id === defaultValues?.Status)?.name,
							}
						}
				...
				<div className=" col-span-2 pt-4 border-t border-main">
					{(RoleID === 1 || RoleID === 3) && (
						<IconButton
					...
					<IconButton onClick={() => router.back()} showLoading icon="fas fa-reply-all" title="Trở về" toolip="" />
				</div>
			</div>

			<div className="col-span-7 tableBoxPag gap-4 h-fit ml-6 grid">
				<div className="tableBox grid p-5 gap-4">
					<div className="col-span-10 text-base font-bold py-2 uppercase border-b border-main">Danh sách sản phẩm</div>
					<div className="col-span-10 pb-4">
						<DepositDetail data={[defaultValues]} />
					</div>
				</div>
				<div className="tableBox grid p-5 gap-4">
					<div className="col-span-2 text-base font-bold py-2 uppercase border-b border-main">Chi tiết đơn hàng</div>
					...
					<div className="col-span-1 pb-4">
						<FormInputNumber
							...
							control={control}
							name="FeeWeightPerKg"
							label={`Đơn giá cân nặng (VNĐ) (CK: ${defaultValues?.FeeWeightCK ?? 0}%)`}
							placeholder=""
							required={false}
							callback={(val) => {
								...
								handleSetValue(
									"DeliveryPrice",
									defaultValues?.FeeWeightCK > 0 ? value * ((100 - defaultValues?.FeeWeightCK) / 100) : value
								);
								handleSetNewTotal();
							}}
						/>
