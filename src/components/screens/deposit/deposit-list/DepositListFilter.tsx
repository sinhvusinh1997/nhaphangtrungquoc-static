...
import {FilterInput, FilterRangeDate, FilterSelect, IconButton} from "~/components";
import {EOrderStatusData, ESearch3Data, orderStatus2Data, orderStatusData, search3Data} from "~/configs/appConfigs";
...
const CollapsePanelHeader = ({setActiveKey, activeKey, handleExporTExcel}) => {
	return (
...
export const DepositListFilter: React.FC<TProps> = ({handleFilter, handleExporTExcel, numberOfOrder}) => {
	...
	return (
		<Collapse
			...
			accordion={true}
			expandIcon={() => (
				<CollapsePanelHeader setActiveKey={setActiveKey} activeKey={activeKey} handleExporTExcel={handleExporTExcel} />
			)}
			activeKey={activeKey}
		>
			<Collapse.Panel header={""} key="1">
				<div className="xl:grid xl:grid-cols-5 xl:gap-4 py-6 px-4">
					<div className="col-span-1 xl:mb-0 mb-4">
						<FilterSelect
							data={[...search3Data.slice(0, 1), ...search3Data.slice(1, 2), ...search3Data.slice(2, 3)]}
							isClearable
							label="Tìm kiếm theo"
