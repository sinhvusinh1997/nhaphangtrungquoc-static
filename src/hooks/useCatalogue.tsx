...
export const useCatalogue = ({
	...
	const userSale = useQuery(
		["userSaleCatalogue"],
		async () => await catalogue.getUser({UserGroupId: 7}).then((res) => res.Data),
		{
			staleTime,
			initialDataUpdatedAt,
			// onError,
	...
	const userOrder = useQuery(
		["userOrderCatalogue"],
		async () => await catalogue.getUser({UserGroupId: 4}).then((res) => res.Data),
		{
			staleTime,
			initialDataUpdatedAt,
			// onError,
	...
	const client = useQuery(
		["clientCatalogue"],
		async () => await catalogue.getUser({UserGroupId: 2}).then((res) => res.Data),
		{
			staleTime,
			initialDataUpdatedAt,
			// onError,
